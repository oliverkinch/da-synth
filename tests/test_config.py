...
def test_qa_config_loads() -> None:
    cfg = load_config(CONFIGS_DIR / "qa" / "danish_wikipedia.yaml")
...
def test_summarization_config_loads() -> None:
    cfg = load_config(CONFIGS_DIR / "summarization" / "eur_lex_sum.yaml")
    assert cfg.task == Task.SUMMARIZATION
    assert cfg.filters.min_assistant_tokens == 50


def test_translation_config_loads() -> None:
    cfg = load_config(CONFIGS_DIR / "translation" / "eur_lex_en_da.yaml")
...
def test_translation_da_en_skips_language_check() -> None:
    cfg = load_config(CONFIGS_DIR / "translation" / "eur_lex_da_en.yaml")
...
def test_render_text_template() -> None:
    ...
    result = cfg.render_text({"title": "Min artikel", "text": "Noget indhold."})
...
def test_render_text_column() -> None:
    ...
    result = cfg.render_text({"text": "Hej verden"})
