...
from synth_da.config import DatasetConfig, Task, load_config
...
def test_translation_config_loads() -> None:
    cfg = load_config(path=CONFIGS_DIR / "translation" / "eur_lex.yaml")
...
def test_render_seed_text_strips_birth_parenthetical() -> None:
    cfg = DatasetConfig(
        task=Task.QA,
        seed_dataset="test/ds",
        text_column="text",
    ...
    row = {"text": "Nicklas Bendtner (født 16. januar 1988) er en fodboldspiller."}
    result = cfg.render_seed_text(row=row)
    assert result == "Nicklas Bendtner er en fodboldspiller."
